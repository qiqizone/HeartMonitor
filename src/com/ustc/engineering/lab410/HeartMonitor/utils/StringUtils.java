...
//			result = URLDecoder.decode(result);
	...
	public static boolean isLettersEqual(String compa,String compb){
		if(compa==null || compb==null)return false;
	...
	public static String getParameterFromUrl(String url, String key){
		...
		if(url != null && url.contains("?")){
			...
			if(keyIndex > 0)
			{
				int valueBeginIndex = keyIndex + 2 + key.length();
				int valueEndIndex = url.indexOf("&", valueBeginIndex);
				if(valueEndIndex < 0)
					valueEndIndex = url.length();
				value = url.substring(valueBeginIndex, valueEndIndex);
			}
		}
		return value;
	}
